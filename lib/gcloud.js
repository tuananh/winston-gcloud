...
var GoogleCloudLogging = function(options) {
    ...
    options.batch_delay         = options.batch_delay || 0
    options.batch_delay_timeout = options.batch_delay_timeout || Infinity
    ...
    this.queue = new Queue(function(batch, cb) {
        ...
        cb()
    }, { batchSize: options.batch_size, batchDelay: options.batch_delay, batchDelayTimeout: options.batch_delay_timeout})

    // batchSize: 50, batchDelay: 5000, batchDelayTimeout: 1000
    // > the queue will wait for 50 items to fill up in 5s or process the queue if no new tasks were added in 1s.
}
